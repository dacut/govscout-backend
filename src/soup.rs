...
use {
    html5ever::{parse_document, tendril::TendrilSink},
    markup5ever_rcdom::RcDom,
    std::io::{Read, Result as IoResult},
};
...
pub use self::{find::QueryBuilder, node_ext::NodeExt, qb_ext::QueryBuilderExt};

/// Create a new `RcDom` instance from a string slice
///
/// # Example
///
/// ```rust
...
/// "#;
///
/// let soup = parse_document(html);
/// #   Ok(())
/// # }
/// ```
pub fn parse_html_str(html: &str) -> RcDom {
    parse_document(RcDom::default(), Default::default()).from_utf8().one(html.as_bytes())
}

/// Create a new `RcDom` instance from something that implements `Read`
///
...
/// # }
/// ```
pub fn parse_html_reader<R: Read>(mut reader: R) -> IoResult<RcDom> {
    parse_document(RcDom::default(), Default::default()).from_utf8().read_from(&mut reader)
}

#[cfg(test)]
mod tests {
    use {
        super::{parse_html_str, NodeExt},
...
"#;
    ...
    #[test]
    fn find() {
        let soup = parse_html_str(TEST_HTML_STRING);
    ...
    fn find_all() {
        let soup = parse_html_str(TEST_HTML_STRING);
        let result = soup.tag("p").find_all().map(|p| p.text()).collect::<Vec<_>>();
