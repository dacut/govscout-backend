use {
    ...
    std::{fmt, marker::PhantomData, rc::Rc},
};

use crate::soup::{attribute, pattern::Pattern};
    ...
    P: Pattern + fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("TagQuery").field("inner", &self.inner).finish()
    }
}
...
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AttrQuery").field("key", &self.key).field("value", &self.value).finish()
    }
}
...
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("QueryWrapper").field("inner", &self.inner).field("next", &self.next).finish()
    }
}
