use {
    markup5ever_rcdom::{Handle, Node, NodeData},
    std::collections::BTreeMap,
};

/// Adds some convenience methods to the `html5ever::Node` type
    ...
    fn attrs(&self) -> BTreeMap<String, String> {
        ...
        match node.data {
            NodeData::Element {
                ...
                let attrs = attrs.borrow();
                attrs.iter().map(|attr| (attr.name.local.to_string(), attr.value.to_string())).collect()
            }
            _ => BTreeMap::new(),
        }
    }
...
mod tests {
    use {
        crate::soup::{parse_html_str, NodeExt, QueryBuilderExt},
        std::collections::BTreeMap,
    };

    #[test]
    fn name() {
        let soup = parse_html_str("<b>some text</b>");
        let b = soup.tag("b").find().expect("Couldn't find tag 'b'");
        let name = b.name();
        assert_eq!(name, "b");
    }
    ...
    #[test]
    fn get() {
        let soup = parse_html_str(r#"<div class="one two"></div>"#);
    ...
    #[test]
    fn attrs() {
        let soup = parse_html_str(r#"<div class="one two" id="some-id"></div>"#);
        let div = soup.tag("div").find().expect("Couldn't find tag 'div'");
        let attrs = div.attrs();
    ...
    #[test]
    fn display() {
        let soup = parse_html_str(r#"<div class="foo bar" id="baz"></div>"#);
        let div = soup.tag("div").find().expect("Couldn't find tag 'div'");
        assert_eq!(div.display(), r#"<div class="foo bar" id="baz"></div>"#);

        let soup = parse_html_str(r#"<div class="foo bar" id="baz"><b>SOME TEXT</b></div>"#);
        let div = soup.tag("div").find().expect("Couldn't find tag 'div'");
        assert_eq!(div.display(), r#"<div class="foo bar" id="baz"><b>SOME TEXT</b></div>"#);

        let soup = parse_html_str(r#"<div class="foo bar" id="baz"><b>SOME TEXT <!-- and a comment --></b></div>"#);
        let div = soup.tag("div").find().expect("Couldn't find tag 'div'");
